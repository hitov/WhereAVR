...
*/
{
	...
	if (rxbytes[11] != 0x40) ax25sendByte(rxbytes[11]>>1);	// 5th char?
	else pad += 1;											// Or just pad with a space

	if (rxbytes[12] != 0x40) ax25sendByte(rxbytes[12]>>1);	// 5th char?
	else pad += 1;											// Or just pad with a space

	rxbytes[13] = (rxbytes[13]>>1) - 64;			// Convert SSID to ASCII
	...
	ax25sendByte('c');
	ax25sendByte('k');
	ax25sendString(&(rxbytes[msg_start + 13]));	// Send ack message number
	return;
